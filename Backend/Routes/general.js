...
const createGeneral = (req, res) => {
  ...
  if ( !user_id || !title || !content)
    return res.status(400).json({ code: 400, message: "user_id, title, content are required" });
    ...
    connection.query(userQuery, [user_id], (userErr, userResults) => {
      ...
      connection.query(GeneralforumQuery, [user_id, title, content], (err, result) => {
          if (err) {
            console.error("database Generalforum query error: ", err);
            return res.status(500).json({ code: 500,  message: "database Generalforum query error" });
          }
          res.status(201).json({ code: 201, message: "general created successfully" });
        }
      );
    });
};
...
const getGeneralDetails = (req, res) => {
  ...
  const updateViewQuery = 'UPDATE Generalforum SET view = view + 1 WHERE id = ?';
  connection.query(updateViewQuery, [general_id], (updateErr, updateResults) => {
    if(updateErr) {
      console.error("database update view error: ", updateErr);
      return res.status(500).json({ code: 500, message: "database update view error" });
    }
...
const updateGeneral = (req, res) => {
  ...
  if (!general_id || !title || !content)
    return res.status(400).json({ code: 400, message: "general_id, title, content is required" });
  ...
  connection.query(query, [title, content, general_id], (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ code: 500, message: "database update Generalforum query error" });
    }
    if (results.affectedRows === 0)
      return res.status(404).json({ code: 404, message: "general not found" });

    res.status(200).json({ code: 200, message: "general updated successfully" });
  });
};
  ...
  connection.query(commentQuery, [general_id], (commentErr, commentResults) => {
    ...
    connection.query(forumQuery, [general_id], (err, results) => {
      if (err) {
        ...
        return res.status(500).json({code: 500, message: "database general delete query error"});
      }
      if (results.affectedRows === 0)
        return res.status(404).json({ code: 404, message: "general not found" });

      res.status(200).json({ code: 200, message: "general deleted successfully" });
    });
  });
};
