...
const createQuestion = (req, res) => {
  ...
  if (!api_id || !user_id || !title || !content)
    return res
      .status(400)
      .json({ error: "api_id, user_id, title, content are required" });
  ...
  connection.query(apiCheckQuery, [api_id], (apiErr, apiResults) => {
    if (apiErr) {
      ...
      return res
        .status(500)
        .json({ code: 500, message: "database error checking API ID" });
    }
...
const getQuestionDetails = (req, res) => {
  ...
  if (!question_id)
    return res
      .status(400)
      .json({ code: 400, message: "question_id is required" });
...
const updateQuestion = (req, res) => {
  ...
  if (!question_id || !title || !content)
    return res
      .status(400)
      .json({ code: 400, message: "question_id, title, content is required" });
  ...
  connection.query(query, [title, content, question_id], (err, results) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ code: 500, message: "database update QAforum query error" });
    }
    ...
    res
      .status(200)
      .json({ code: 200, message: "question updated successfully" });
  });
};
