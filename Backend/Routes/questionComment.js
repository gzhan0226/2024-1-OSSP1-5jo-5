...
const validateId = (table, id, callback) => {
  ...
  if (table === "Users")
    query = "select count(*) as count from ?? where user_id = ?";
...
const createComment = (req, res) => {
  ...
  if (!question_id || !user_id || !content)
    return res.status(400).json({
      code: 400,
      message: "question_id, user_id, content are required",
    });
  ...
  validateId("QAforum", question_id, (err, isQAIdValid) => {
    ...
    if (!isQAIdValid)
      return res
        .status(400)
        .json({ code: 400, message: "Invalid question_id" });
    ...
    validateId("Users", user_id, (err, isUserIdValid) => {
      ...
      const query =
        "insert into QAcomment (forum_id, user_id, content) values (?, ?, ?)";
      connection.query(
        query,
        ...
        (err, results) => {
          ...
          return res
            .status(201)
            .json({ code: 201, message: "comment created successfully" });
        }
      );
    });
  });
...
const getComments = async (req, res) => {
  ...
  if (question_id && user_id)
    return res.status(400).json({
      code: 400,
      message: "only one of question_id and user_id is possible",
    });

  let query = `select * from QAcomment`;
  let countQuery = `select count(*) as total from QAcomment`;
  let queryParams;
  try {
    if (question_id) {
      const questionCount = `select count(*) as total from QAforum where id = ?`;
      const [count] = await connection.query(questionCount, question_id);
      if (count.total == 0)
        return res
          .status(400)
          .json({ code: 400, message: "invalid question_id" });
      query += ` where forum_id = ?`;
      countQuery += ` where forum_id = ?`;
...
const updateComment = (req, res) => {
  ...
  if (!comment_id || !content)
    return res
      .status(400)
      .json({ code: 400, message: "comment_id, content are required" });
  ...
  validateId("QAcomment", comment_id, (err, isValid) => {
    ...
    if (!isValid)
      return res.status(400).json({ code: 400, message: "Invalid comment_id" });
    ...
    connection.query(query, comment_id, (err, results) => {
      ...
      if (results[0].accepted === 1)
        return res
          .status(400)
          .json({ code: 400, message: "cannot update accepted comment" });
      ...
      connection.query(updateQuery, [content, comment_id], (err, results) => {
        ...
        return res
          .status(200)
          .json({ code: 200, message: "comment updated successfully" });
      });
    });
  });
};
...
const deleteComment = (req, res) => {
  ...
  if (!comment_id)
    return res
      .status(400)
      .json({ code: 400, message: "comment_id is required" });
  ...
  validateId("QAcomment", comment_id, (err, isValid) => {
    ...
    connection.query(query, [comment_id], (err, results) => {
      ...
      if (results[0].accepted === 1)
        return res
          .status(400)
          .json({ code: 400, message: "cannot delete accepted comment" });
      ...
      connection.query(deleteQuery, [comment_id], (err, results) => {
        ...
        return res
          .status(200)
          .json({ code: 200, message: "comment deleted successfully" });
      });
    });
  });
};
...
const acceptComment = (req, res) => {
  ...
  validateId("QAcomment", comment_id, (err, isValid) => {
    ...
    connection.query(QAforumQuery, [comment_id], (err, QAforumResults) => {
      ...
      if (QAforumResults[0].accepted === 1)
        return res
          .status(400)
          .json({ code: 400, message: "comment is already accepted" });
      ...
      connection.query(query, [comment_id], (err, results) => {
        ...
        const levelupQuery =
          "update Users set levelpoint = levelpoint + 1 where user_id = ?";
        connection.query(
          levelupQuery,
          [QAforumResults[0].user_id],
          (err, results) => {
            if (err) {
              console.error("database query err: ", err);
              return res
            ...
            return res
              .status(200)
              .json({ code: 200, message: "comment accepted successfully" });
          }
        );
      });
    });
