...
const ReadQnA = () => {
  ...
  const post = {
    ...
    date: "2024-06-01",
    content:
      "React 애플리케이션에서 OAuth 인증을 통합하려고 합니다. 이를 달성하는 방법에 대한 자세한 가이드나 모범 사례를 제공해 주실 수 있나요?",
  };
  ...
  const [answers, setAnswers] = useState([
    {
      ...
      date: "2024-06-02",
      content:
        "React에서 OAuth를 통합하려면 react-oauth2-hook이나 react-oauth 같은 라이브러리를 사용할 수 있습니다. 안전하게 OAuth 토큰을 처리하기 위해 백엔드 서비스를 만들어야 합니다.",
      showReplyBox: false,
      isAccepted: true,
      replies: [
        {
          ...
          date: "2024-06-03",
          content:
            "추천해 주셔서 감사합니다. react-oauth2-hook을 사용하는 예시를 제공해 주실 수 있나요?",
        },
      ],
    },
  ]);
  ...
  const hasUserRepliedToAnswer = (answer) =>
    answer.replies.some((reply) => reply.authorId === userId);
  ...
  const sortedAnswers = answers
    ...
    .sort(
      (a, b) =>
        b.isAccepted - a.isAccepted || new Date(b.date) - new Date(a.date)
    );
                          ...
                          <>
                            ...
                            {userId === reply.authorId && (
                              <S.ReplyActions>
                                <button
                                  onClick={() =>
                                    handleReplyEdit(answer.id, reply)
                                  }
                                >
                                  수정
                                </button>
                                <button
                                  onClick={() =>
                                    handleReplyDelete(answer.id, reply.id)
                                  }
                                >
                                  삭제
                                </button>
